...
// 文章表单
export const contentGroupForm = (isB2B: boolean, cateType: ContentCateType, onChange?: (params: any) => void): FormConfig => {
  ...
  const config: FormConfig = {
    ...
    useLabelCol: { span: 4 },
    children: [
      { formItemWidth: '100%', label: '分组名称',
      onChange: (...args: any) => {}, name: 'name', type: FormType.Input, required: true, placeholder: '请输入分组名称，2~8个字', minLength: 2, maxLength: 8, patternList: [{ pattern: /^[\s\S]{2,8}$/, message: '2～8个字' }], showCount: true},
      ...
      { formItemWidth: '100%', label: 'SEO标题', onChange: (...args: any) => {
        const [newValue, form] = args
          onChangeValue({newValue, form, name: 'seoT'})
