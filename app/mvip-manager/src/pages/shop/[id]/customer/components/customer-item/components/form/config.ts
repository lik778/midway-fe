...
export const CustomerSetFormConfigFn: (key: string | number, imageChange: (...arg: any) => any) => FormConfig = (key, imageChange) => ({
  ...
  children: [
    ...
    {
      label: '上传图片', name: 'urlImg', maxLength: 1, type: FormType.ImgUpload, images: [{ uploadType: 2, text: '', name: 'urlImg', maxSize: 3, rule: [{ required: true, message: `请上传图片` }], cropProps: { aspectRatio: 595 / 222 }, aspectRatio: 595 / 222 }],
      required: true, tip: '图片格式：jpg、jpeg、png，大小不超过3M，建议最佳尺寸595*222，上传图片后，请选择合适的字体颜色。', imagesTipPosition: 'right', onChange: imageChange
    },
  ]
}
)