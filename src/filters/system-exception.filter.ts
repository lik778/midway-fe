...
export class SystemExceptionFilter implements ExceptionFilter {
    ...
    const newSystemException = new SystemException(HttpStatus.INTERNAL_SERVER_ERROR, false, exception, 'system-exception.filter SystemExceptionFilter SystemException 1', JSON.stringify({ url: req.originalUrl, headers: req.headers }))
